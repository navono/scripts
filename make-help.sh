...
thor 切换推理后端 (共用端口 8301, 一次一个):
  make vllm    切换到 vLLM (Qwen3.8-27B NVFP4 + MTP)
  make ds      切换到 dsv4flash (DeepSeek-V4-Flash Q2)
  make sglang  切换到 SGLang + DSpark (Qwen3.8-27B)
  make llama   切换到 llama.cpp (Qwen3.8-27B Q4_K_M)
  make flashnext 切换到 Qwen3.8-Flash-Next (llama.cpp qwen4exp)
  make stop    停止 thor 全部服务

rtx4090 本机服务 (共用双卡显存, 一次一个):
  make flash / flash-stop          Qwen3.8-Flash-Next-Uncensored (端口 8301)
  make coldfusion / coldfusion-stop  Qwen3.8-27B Cold-Fusion NEO-MTP (端口 12234)
